...
class PositionalEncoding(nn.Module):
    ...
    def forward(self, x):
        ...
        x = x + (self.pe[:, : x.shape[1], :]).requires_grad_(
            False
        )  # We also tell the model that we don't want to learn this positional encoding
...
class LayerNormalization(nn.Module):
    def __init__(self, eps: float = 10**-6) -> None:
        ...
        self.alpha = nn.Parameter(
            torch.ones(1)
        )  # Multiplicative # nn.Parameter makes this parameter learnable
...
def build_transformer(src_vocab_size: int, tgt_vocab_size: int, src_seq_len: int, tgt_seq_len: int, d_model: int = 512, N: int = 6, h: int = 8, dropout: float = 0.1, d_ff: int = 2048):
    ...
    decoder = Decoder(nn.ModuleList(encoder_blocks))
